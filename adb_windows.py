...
try:
  subprocess.Popen('adb connect 127.0.0.1:7555',shell=True)
  subprocess.Popen('adb shell am start -n cn.xuexi.android/com.alibaba.android.rimet.biz.home.activity.HomeActivity',
                   shell=True)
except:
...
for i in range(9):
  ...
  subprocess.Popen('adb shell input tap 827 1599', shell=True) #防止进入图片
  time.sleep(sleep_time)
  ...
  # 评论
  if i < 2:
    subprocess.Popen('adb shell input tap 159 1930', shell=True)
    time.sleep(5)
    # for i in range(40):
    #   subprocess.Popen('adb shell input keyevent 67', shell=True)
    commentKey = random.randint(1, 5)
    if commentKey == 1:
      subprocess.Popen('adb shell am broadcast -a ADB_INPUT_TEXT --es msg "中华人民共和国万万岁！"', shell=True)
    elif commentKey == 2:
      subprocess.Popen('adb shell am broadcast -a ADB_INPUT_TEXT --es msg "富强、民主、文明、和谐。为天地立心！"', shell=True)
    elif commentKey == 3:
      subprocess.Popen('adb shell am broadcast -a ADB_INPUT_TEXT --es msg "中华民族崛起！为中光滑之崛起而读书！"', shell=True)
    elif commentKey == 4:
    ...
    time.sleep(2)
    subprocess.Popen('adb shell input tap 1020 1795', shell=True)
    time.sleep(5)
...
time.sleep(sleep_time)
subprocess.Popen('adb shell am force-stop cn.xuexi.android',shell=True)
